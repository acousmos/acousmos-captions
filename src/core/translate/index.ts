...
async function translateChunk(
	...
	const idsEcho =
		...
		batch.every((it, k) => parsed[k]!.i === it.i && parsed[k]!.t.length > 0);
	const srcEcho =
		idsEcho &&
		batch.every(
			(it, k) => normalizeSrc(parsed[k]!.src ?? "") === normalizeSrc(it.t),
		);
	if (srcEcho) {
	...
	const rightPrev = [...prevSource, ...left.map((x) => x.t)].slice(
		-CONTEXT_LINES,
	);
