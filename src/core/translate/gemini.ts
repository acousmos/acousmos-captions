...
export const gemini: TranslateProvider = {
	id: "gemini",
	translateBatch: (items, ctx, opts) =>
		openaiCompat.translateBatch(items, ctx, { ...opts, baseUrl: GEMINI_BASE }),
	testKey: (key, opts) =>
		openaiCompat.testKey(key, { model: opts.model, baseUrl: GEMINI_BASE }),
};
