...
export const soniox: AsrProvider = {
  ...
  async transcribe(audio: AudioPayload, opts: AsrOptions): Promise<Utterance[]> {
    ...
    const ext = audio.mime.includes('mp2t') ? 'ts' : audio.mime.includes('aac') ? 'aac' : 'mp4'
    form.append('file', new Blob([audio.bytes as unknown as BlobPart], { type: audio.mime }), `audio.${ext}`)
    ...
    if (!upload.ok) throw new JobError('err_asr_failed', `Soniox upload HTTP ${upload.status}: ${await readErrorBody(upload)}`)
    ...
    try {
      ...
      if (!create.ok) throw new JobError('err_asr_failed', `Soniox create HTTP ${create.status}: ${await readErrorBody(create)}`)
      transcriptionId = ((await create.json()) as { id: string }).id

      // 3) Poll until completed.
      const deadline = Date.now() + POLL_TIMEOUT_MS
      for (;;) {
        if (opts.signal?.aborted) throw new JobError('err_asr_failed', 'aborted')
        if (Date.now() > deadline) throw new JobError('err_asr_failed', 'Soniox polling timed out')
        const st = await fetchRetry(`${API}/transcriptions/${transcriptionId}`, { headers: auth }, { signal: opts.signal })
        if (!st.ok) throw new JobError('err_asr_failed', `Soniox status HTTP ${st.status}`)
        const status = (await st.json()) as { status: string; error_message?: string }
        if (status.status === 'completed') break
        if (status.status === 'error') throw new JobError('err_asr_failed', status.error_message ?? 'Soniox transcription error')
        await new Promise((r) => setTimeout(r, POLL_INTERVAL_MS))
      }

      // 4) Fetch transcript tokens.
      const tr = await fetchRetry(`${API}/transcriptions/${transcriptionId}/transcript`, { headers: auth }, { signal: opts.signal })
      if (!tr.ok) throw new JobError('err_asr_failed', `Soniox transcript HTTP ${tr.status}`)
      ...
      void cleanup(`${API}/transcriptions/${transcriptionId ?? ''}`, auth, Boolean(transcriptionId))
      void cleanup(`${API}/files/${fileId}`, auth, true)
    }
  },

  async testKey(key: string): Promise<boolean> {
    const res = await fetch(`${API}/files?limit=1`, { headers: { Authorization: `Bearer ${key}` } })
    return res.ok
  },
}

async function cleanup(url: string, auth: Record<string, string>, enabled: boolean): Promise<void> {
  if (!enabled) return
  try {
