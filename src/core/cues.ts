...
export function buildCues(
	...
	opts: CueOptions = DEFAULT_CUE_OPTIONS,
): Cue[] {
	...
	for (const w of words) {
		if (bucket.length > 0) {
			...
			const wouldExceed =
				chars + w.text.length + 1 > opts.maxChars ||
				w.end - start > opts.maxDur;
			const sentenceBoundary =
				SENTENCE_END.test(prev.text) &&
				(chars >= opts.softMin || gap >= opts.shortGap);
...
function joinWords(words: Word[]): string {
	...
	for (const w of words) {
		...
		const cjk = /[　-〿㐀-鿿＀-￯]/;
		out +=
			cjk.test(out.slice(-1)) && cjk.test(w.text[0] ?? "")
				? w.text
				: ` ${w.text}`;
	}
	return out.trim();
