 ...
 */
export function pickAudioSource(master: MasterPlaylist): AudioSourcePick | null {
  ...
  if (master.variants.length > 0) {
    const lowest = [...master.variants].sort((a, b) => a.bandwidth - b.bandwidth)[0]
...
export function mimeForPlaylist(media: MediaPlaylist, kind: 'audio' | 'muxed'): string {
