...
function syncRangeLabels(): void {
	$("src-scale-val").textContent =
		`${Math.round(Number(els.srcScale.value) * 100)}%`;
	$("tgt-scale-val").textContent =
		`${Math.round(Number(els.tgtScale.value) * 100)}%`;
	$("bg-opacity-val").textContent =
		`${Math.round(Number(els.bgOpacity.value) * 100)}%`;
}
...
bindTest("test-anthropic", () =>
	anthropic.testKey(els.anthropicKey.value.trim(), {
		model: els.anthropicModel.value.trim(),
	}),
);
bindTest("test-gemini", () =>
	gemini.testKey(els.geminiKey.value.trim(), {
		model: els.geminiModel.value.trim(),
	}),
);
