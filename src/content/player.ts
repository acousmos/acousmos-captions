...
export class PlayerController {
	...
	constructor(
		...
		this.container.addEventListener("mousemove", this.revealPill, {
			passive: true,
		});
	...
	private pause(): void {
		...
		this.setPill(
			this.cues.length > 0 ? "pill_on" : "pill_idle",
			this.cues.length > 0 ? "live" : "idle",
		);
	...
	private async beginJob(
		...
		if (!forceAsr) {
			try {
				nativeUtterances =
					(await getNativeUtterances(
						this.video,
						this.settings.asr.sourceLang,
					)) ?? undefined;
			} catch {
		...
		const port = chrome.runtime.connect({
			name: `${JOB_PORT_PREFIX}${this.mediaId}`,
		});
		this.port = port;
		port.onMessage.addListener((ev: JobEvent) => this.onEvent(ev));
		port.onDisconnect.addListener(() => {
			if (this.port === port) this.port = null;
			if (!this.disposed && this.state === "working")
		...
		switch (ev.kind) {
			...
			case "job/progress": {
				if (ev.progress.errorKey) {
					...
					ev.progress.ratio !== undefined &&
					this.cues.length > 0 &&
					ev.progress.ratio < 1
				) {
					// Captions are already showing; the rest of a long video streams in.
					this.setPillRaw(
						`${t("pill_idle")} ${Math.round(ev.progress.ratio * 100)}%`,
						"live",
					);
				} else if (this.cues.length === 0) {
					if (ev.progress.phase === "fetching_audio")
						this.setPill("pill_capturing", "working");
					else if (ev.progress.phase === "transcribing")
						this.setPill("pill_transcribing", "working");
					else if (ev.progress.phase === "translating")
						this.setPill("pill_translating", "working");
				}
				break;
			}
			case "job/utterances": {
	...
	private toggleMenu(): void {
		...
		menu.appendChild(
			this.menuRadio(t("opt_mode_bilingual"), mode === "bilingual", () =>
				this.setMode("bilingual"),
			),
		);
		menu.appendChild(
			this.menuRadio(t("opt_mode_source"), mode === "source", () =>
				this.setMode("source"),
			),
		);
		menu.appendChild(
			this.menuRadio(t("opt_mode_target"), mode === "target", () =>
				this.setMode("target"),
			),
		);
		menu.appendChild(this.menuSep());
		...
		menu.appendChild(
			this.menuItem(t("menu_export_bilingual"), () =>
				downloadSrt(this.cues, "bilingual", this.mediaId, lang, eng),
			),
		);
		menu.appendChild(
			this.menuItem(t("menu_export_source"), () =>
				downloadSrt(this.cues, "source", this.mediaId, lang, eng),
			),
		);
		menu.appendChild(
			this.menuItem(t("menu_export_target"), () =>
				downloadSrt(this.cues, "target", this.mediaId, lang, eng),
			),
		);
		menu.appendChild(this.menuSep());
		menu.appendChild(
			this.menuItem(t("menu_retranslate"), () => this.start(true)),
		);
		// Escape hatch when the video's own captions are poor: force ASR.
		menu.appendChild(
			this.menuItem(t("menu_force_asr"), () => this.start(true, true)),
		);
		menu.appendChild(
			this.menuItem(t("menu_settings"), () => {
				void chrome.runtime
					.sendMessage({ kind: "options/open" })
					.catch(() => undefined);
			}),
		);
	...
	private setMode(mode: Settings["display"]["mode"]): void {
		this.settings = {
			...this.settings,
			display: { ...this.settings.display, mode },
		};
	...
	private positionMenu(menu: HTMLDivElement): void {
		...
		if (top + mh > window.innerHeight - margin) {
			...
			top = Math.max(
				margin,
				Math.min(r.top - 6 - mh, window.innerHeight - margin - mh),
			);
		}
		const left = Math.max(
			margin,
			Math.min(r.right - mw, window.innerWidth - margin - mw),
		);
