...
export function attachPort(port: chrome.runtime.Port, req: JobRequest): void {
	void (async () => {
		...
		if (existing && !req.force) {
			...
			send(port, {
				kind: "job/progress",
				progress: { phase: existing.snapshot.phase },
			});
...
async function runJob(
	...
	if (!req.force) {
		const cached = await cacheGet(
			mediaId,
			targetLang,
			cacheIdentity(settings, sourceModeOf(req)),
		);
		if (cached) {
	...
	const processWindow = async (
		...
		const windowCues = buildCues(offsetUtterances(utterances, startTime)).map(
			(c, i) => ({
				...c,
				id: cues.length + i,
			}),
	...
	if (req.resumeCues && req.resumeCues.length > 0) {
		...
		broadcast(job, {
			kind: "job/progress",
			progress: { phase: "translating", ratio: 1 },
		});
		...
		broadcast(job, {
			kind: "job/debug",
			info: `native captions: ${req.nativeUtterances.length} cues`,
		});
		setPhase(job, "translating");
		...
		broadcast(job, {
			kind: "job/debug",
			info: `plans: ${plans.map((p) => p.label).join(", ")}`,
		});
		for (const plan of plans) {
			if (signal.aborted) return;
			try {
				const payload = await plan.getWindow(0);
				const head = [...payload.bytes.subarray(0, 4)]
				...
				broadcast(job, {
					kind: "job/debug",
					info: `OK "${plan.label}" → ${utts.length} utterances`,
				});
				break;
			} catch (e) {
				...
				broadcast(job, {
					kind: "job/debug",
					info: `rejected "${plan.label}": ${(e as Error).message}`,
				});
		...
		if (!active)
			throw lastErr instanceof Error
				? lastErr
				: new JobError("err_audio_fetch");

		setPhase(job, "translating");
		await processWindow(firstUtterances, active.startTimes[0] ?? 0);
		broadcast(job, {
			kind: "job/progress",
			progress: { phase: "translating", ratio: 1 / active.total },
		});
		...
		for (let i = 1; i < active.total; i++) {
			...
			broadcast(job, {
				kind: "job/progress",
				progress: { phase: "translating", ratio: (i + 1) / active.total },
			});
		}
	}
...
function sourceModeOf(req: JobRequest): "native" | "asr" {
	return req.nativeUtterances && req.nativeUtterances.length > 0
		? "native"
		: "asr";
}
