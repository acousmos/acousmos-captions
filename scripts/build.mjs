...
const builds = [
	...
	{
		...common,
		entryPoints: ["src/bg/index.ts"],
		outfile: `${outdir}/bg.js`,
		format: "esm",
	},
...
function startReloadServer(port) {
	...
	server.listen(port, "127.0.0.1", () =>
		console.log(`[reload] ws://127.0.0.1:${port}`),
	);
	...
	return () => {
		...
		timer = setTimeout(() => {
			send("reload");
			if (sockets.size > 0)
				console.log(`[reload] notified ${sockets.size} client(s)`);
			else
				console.log(
					"[reload] no clients connected (open/refresh an X tab to wake the SW)",
				);
		}, 150);
	};
}
...
if (watch) {
	...
	const contexts = await Promise.all(
		builds.map((b) => esbuild.context({ ...b, plugins: [onEnd] })),
	);
	...
	for (const dir of ["public", "src"])
		fsWatch(dir, { recursive: true }, (_e, file) => {
			if (file && /\.(css|html|json|png|svg)$/.test(file)) recopy();
		});
	console.log(
		"[build] watching src + assets; the extension hot-reloads on change",
	);
} else {
	await Promise.all(builds.map((b) => esbuild.build(b)));
	await copyStatic();
	if (!existsSync(`${outdir}/manifest.json`))
		throw new Error("manifest.json missing from dist");
