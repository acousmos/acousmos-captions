...
function syntheticInit(): Uint8Array {
  const esds = box('esds', [
    ...
    0x03, 0x19, 0x00, 0x01, 0x00, // ES_Descriptor: tag,len, ES_ID(2)+flags(1)
    0x04, 0x11, ...new Array(13).fill(0), // DecoderConfigDescriptor: tag,len, 13 bytes
    0x05, 0x02, 0x12, 0x08, // DecoderSpecificInfo: tag,len, ASC
  ])
...
function syntheticFragment(sizes: number[]): Uint8Array {
  ...
  const trun = box('trun', [0, 0, 0x02, 0x00, ...u32(sizes.length), ...sizes.flatMap(u32)]) // flag 0x200 (size)
...
describe('adtsHeader', () => {
  it('matches the bytes validated against real X audio (frame len 93)', () => {
    expect([...adtsHeader(CFG, 93)]).toEqual([0xff, 0xf1, 0x50, 0x40, 0x0c, 0x9f, 0xfc])
  })
...
describe('parseAacConfig', () => {
  ...
  it('returns null when there is no mp4a/esds', () => {
    expect(parseAacConfig(Uint8Array.from(box('ftyp', ascii('isom'))))).toBeNull()
  })
})
...
describe('fragmentToAdts', () => {
  ...
  it('produces a stream whose ADTS frame lengths walk exactly to the end', () => {
    ...
    while (pos + 7 <= out.length) {
      ...
      const len = ((out[pos + 3]! & 0x03) << 11) | (out[pos + 4]! << 3) | (out[pos + 5]! >> 5)
      pos += len
      frames++
    }
