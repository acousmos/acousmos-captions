...
import {
	...
	mediaIdFromPoster,
	streamInfoFromUrl,
} from "../src/core/mediaid";

describe("mediaIdFromPoster", () => {
	it("extracts id from amplify_video_thumb posters", () => {
		expect(
			mediaIdFromPoster(
				"https://pbs.twimg.com/amplify_video_thumb/1799887766554433221/img/AbCdEf.jpg",
			),
	...
	it("extracts id from ext_tw_video_thumb posters", () => {
		expect(
			mediaIdFromPoster(
				"https://pbs.twimg.com/ext_tw_video_thumb/1234567890123/pu/img/xx.jpg",
			),
	...
	it("flags GIF thumbnails (no audio)", () => {
		expect(
			mediaIdFromPoster("https://pbs.twimg.com/tweet_video_thumb/AbCd123.jpg"),
		).toEqual({ kind: "gif" });
	});

	it("returns null for normal images and empty input", () => {
		expect(
			mediaIdFromPoster("https://pbs.twimg.com/media/AbCd123.jpg"),
		).toBeNull();
...
describe("streamInfoFromUrl", () => {
	it("parses master playlist URLs and recognizes the true master", () => {
		const u =
			"https://video.twimg.com/amplify_video/1799887766554433221/pl/abc.m3u8?tag=14";
		expect(streamInfoFromUrl(u)).toEqual({
	...
	it("treats rendition playlists (codec/res subpath) as non-master", () => {
		expect(
			isMasterPlaylistUrl(
				"https://video.twimg.com/amplify_video/123/pl/avc1/640x360/x.m3u8",
			),
		).toBe(false);
		expect(
			isMasterPlaylistUrl(
				"https://video.twimg.com/amplify_video/123/pl/mp4a/128000/x.m3u8",
			),
		).toBe(false);
		expect(
			isMasterPlaylistUrl(
				"https://video.twimg.com/amplify_video/123/aud/32000.m3u8",
			),
		).toBe(false);
	});

	it("parses mp4 variants with pixel area", () => {
		const info = streamInfoFromUrl(
			"https://video.twimg.com/amplify_video/99/vid/avc1/640x360/file.mp4?tag=1",
		);
	...
	it("ignores unrelated hosts", () => {
		expect(
			streamInfoFromUrl("https://example.com/amplify_video/1/pl/a.m3u8"),
		).toBeNull();
	});
});
