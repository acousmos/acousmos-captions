...
describe("parseAttributes", () => {
	it("handles quoted values containing commas", () => {
		const a = parseAttributes(
			'BANDWIDTH=950000,CODECS="avc1.640020,mp4a.40.2",AUDIO="g"',
		);
...
describe("parseMedia", () => {
	it("extracts init segment, media segments and per-segment durations", () => {
		...
		expect(media.initUri).toBe(
			"https://video.twimg.com/amplify_video/123/aud/init.mp4",
		);
	...
	it("maps container to MIME", () => {
		...
		expect(
			mimeForPlaylist(
				{ segmentUris: ["a/0.ts"], segmentDurations: [1], totalDuration: 1 },
				"muxed",
			),
		).toBe("video/mp2t");
	});
...
describe("planWindows", () => {
	...
	it("returns a single window when total is under windowSec", () => {
		expect(planWindows([4, 4, 4], 60)).toEqual([
			{ startIndex: 0, endIndex: 3, startTime: 0 },
		]);
	});
