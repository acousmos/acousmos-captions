...
describe("extractItems", () => {
	it("parses the canonical shape", () => {
		expect(
			extractItems('{"items":[{"i":0,"t":"你好"},{"i":1,"t":"世界"}]}'),
		).toEqual([
			{ i: 0, t: "你好" },
			{ i: 1, t: "世界" },
		]);
	...
	it("drops malformed entries but keeps valid ones", () => {
		expect(
			extractItems(
				'{"items":[{"i":0,"t":"ok"},{"i":"x","t":1},{"t":"no id"}]}',
			),
		).toEqual([{ i: 0, t: "ok" }]);
	});
...
function fakeProvider(
	impl: TranslateProvider["translateBatch"],
): TranslateProvider {
...
describe("translateCues", () => {
	...
	it("catches a same-count reply that echoes ids correctly but shifts the translations", async () => {
		...
		const shifted: TranslateProvider["translateBatch"] = async (items) => {
			if (items.length === 1)
				return [{ i: items[0]!.i, src: items[0]!.t, t: `译${items[0]!.i}` }];
		...
		const result = await translateCues(cues, fakeProvider(shifted), {
			...baseOpts,
			batchSize: 5,
		});
		expect(result.size).toBe(5);
	...
	it("recovers when the model merges a split sentence (returns fewer, shifted lines)", async () => {
		const merging: TranslateProvider["translateBatch"] = async (items) => {
			...
			return items
				.slice(1)
				.map((it, k) => ({ i: items[k]!.i, src: it.t, t: `译${it.i}` }));
		};
		const result = await translateCues(cues, fakeProvider(merging), {
			...baseOpts,
			batchSize: 5,
		});
		expect(result.size).toBe(5);
	...
	it("stays aligned even when the model never echoes the source (degrades to per-line)", async () => {
		...
		const result = await translateCues(cues, fakeProvider(noSrc), {
			...baseOpts,
			batchSize: 5,
		});
		expect(result.size).toBe(5);
	...
	it("tolerates whitespace/case differences in the echoed source", async () => {
		...
		const provider = fakeProvider(async (items) => {
			...
			return items.map((it) => ({
				i: it.i,
				src: `  ${it.t.toUpperCase()}  `,
				t: `译${it.i}`,
			}));
		});
		const result = await translateCues(cues.slice(0, 3), provider, {
			...baseOpts,
			batchSize: 3,
		});
	...
	it("leaves a cue untranslated when its line never comes back (no throw)", async () => {
		...
		const provider = fakeProvider(async (items) =>
			items
				.filter((it) => it.i !== 2)
				.map((it) => ({ i: it.i, src: it.t, t: `译${it.i}` })),
		);
		const result = await translateCues(cues.slice(0, 3), provider, {
			...baseOpts,
			batchSize: 3,
