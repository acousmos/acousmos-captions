...
describe('buildCues — sentence-aware grouping', () => {
    ...
    const utts: Utterance[] = [
      {
        ...
        words: words([
          ['how', 39, 39.2], ['many', 39.2, 39.4], ['of', 39.4, 39.5], ['you', 39.5, 39.7],
          ['used', 39.7, 40], ['the', 40, 40.1], ['same', 40.1, 40.4], ['AI', 40.4, 40.7],
          ['tool', 40.7, 41], ['to', 41, 41.1], ['deploy', 41.1, 41.6], ['on', 41.6, 41.8],
          ['Google', 41.8, 44.1],
        ]),
      },
      { start: 44.3, end: 45.1, text: 'Cloud?', words: words([['Cloud?', 44.3, 45.1]]) },
    ]
    const cues = buildCues(utts)
    expect(cues).toHaveLength(1)
    expect(cues[0]!.src).toBe('how many of you used the same AI tool to deploy on Google Cloud?')
    ...
    const utts: Utterance[] = [
      {
        ...
        words: words([
          ['This', 0, 0.3], ['is', 0.3, 0.5], ['one', 0.5, 0.8], ['sentence.', 0.8, 1.3],
          ['And', 1.8, 2], ['here', 2, 2.3], ['is', 2.3, 2.5], ['another', 2.5, 3], ['one.', 3, 3.4],
        ]),
      },
    ]
    const cues = buildCues(utts)
    expect(cues.map((c) => c.src)).toEqual(['This is one sentence.', 'And here is another one.'])
  })
    ...
    const utts: Utterance[] = [
      {
        ...
        words: words([
          ['Um.', 0, 0.3], ['So', 0.4, 0.6], ['what', 0.6, 0.8], ['we', 0.8, 0.9],
          ['built', 0.9, 1.2], ['here', 1.2, 1.5], ['is', 1.5, 1.6], ['a', 1.6, 1.7],
          ['dashboard.', 1.7, 2.4],
        ]),
      },
    ]
  ...
  it('splits a run-on at the maxChars cap (boundary is a word timestamp)', () => {
    const spec: [string, number, number][] = Array.from({ length: 40 }, (_, i) => [
      `word${i}`,
      i * 0.3,
      i * 0.3 + 0.25,
    ])
    const cues = buildCues([{ start: 0, end: 12, text: spec.map((s) => s[0]).join(' '), words: words(spec) }])
  ...
  it('breaks on a long pause even without punctuation', () => {
    const utts: Utterance[] = [
      { start: 0, end: 1, text: 'first part', words: words([['first', 0, 0.5], ['part', 0.5, 1]]) },
      { start: 4, end: 5, text: 'second part', words: words([['second', 4, 4.5], ['part', 4.5, 5]]) },
    ]
    const cues = buildCues(utts)
  ...
  it('joins CJK without inserting spaces', () => {
    const cues = buildCues([
      { start: 0, end: 2, text: '你好', words: words([['你好', 0, 1], ['世界', 1, 2]]) },
    ])
