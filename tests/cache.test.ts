...
describe("cache", () => {
	...
	it("misses when the engine differs (so switching provider/model/base re-translates)", async () => {
		...
		expect(
			await cacheGet(
				"m1",
				"zh-CN",
				llmCacheTag(settingsFor({ provider: "gemini" })),
			),
		).toBeNull();
	...
	it("purges only older schema versions, keeping current entries", async () => {
		...
		expect(
			[...store.keys()].some(
				(k) => k.startsWith("cap:v3:") || k.startsWith("cap:v4:"),
			),
		).toBe(false);
		expect(
			await cacheGet("m1", "zh-CN", cacheIdentity(s, "asr")),
		).not.toBeNull(); // current survived
	});
});
...
describe("cacheIdentity (glossary + source are part of cache identity)", () => {
	...
	it("misses after the user edits the glossary (so the change re-translates)", async () => {
		...
		const after = {
			...before,
			asr: { ...before.asr, customTerms: "agent=代理" },
		};
		expect(cacheIdentity(after, "asr")).not.toBe(cacheIdentity(before, "asr"));
		expect(
			await cacheGet("m1", "zh-CN", cacheIdentity(after, "asr")),
		).toBeNull();
	});
	...
	it("keeps native and ASR sources on separate keys (re-transcribe ≠ overwrite native)", async () => {
		...
		expect(
			await cacheGet("m1", "zh-CN", cacheIdentity(s, "native")),
		).not.toBeNull();
	...
	it("ASR source: provider and source language are part of the identity", async () => {
		...
		const asrBase: Settings = {
			...base,
			asr: { ...base.asr, provider: "deepgram", sourceLang: "auto" },
		};
		const otherProvider: Settings = {
			...asrBase,
			asr: { ...asrBase.asr, provider: "soniox" },
		};
		const otherLang: Settings = {
			...asrBase,
			asr: { ...asrBase.asr, sourceLang: "en" },
		};
		expect(cacheIdentity(otherProvider, "asr")).not.toBe(
			cacheIdentity(asrBase, "asr"),
		);
		expect(cacheIdentity(otherLang, "asr")).not.toBe(
			cacheIdentity(asrBase, "asr"),
		);
	});
	...
	it("native source: the ASR provider is irrelevant (no needless re-run)", async () => {
		const base = settingsFor({});
		const deepgram: Settings = {
			...base,
			asr: { ...base.asr, provider: "deepgram" },
		};
		const soniox: Settings = {
			...base,
			asr: { ...base.asr, provider: "soniox" },
		};
		expect(cacheIdentity(soniox, "native")).toBe(
			cacheIdentity(deepgram, "native"),
		);
	});
});
